...
- 📊 **Data**: Subfolder **[0. DATA](./0.%20DATA/)** with downloadable datasets
...
- ✨ **Markdown-first approach**: All content co-located with code
  ...
  - 📄 Export to PDF/HTML/PPT using Markdown Preview Enhanced
  - 🤖 AI-friendly format—markdown is how agents exchange context
  - 🛠️ Alternatives: pandoc, marp (though marp lacks mermaid support)
...
Most execution happens in Docker, so Windows works too. I recommend *nix if pressed. 💬 Email: [pradeep@automationpractice.com](pradeep@automationpractice.com)
...
- 📓 **Notebook first run**: Install ipykernel when prompted ![alt text](image-3.png)
- ▶️ **Python in interactive window**: Install Python + Jupyter extensions; enable in Settings
- 🌐 **Markdown export to PDF**: Set Chrome path in Markdown Enhanced settings ![chrome path](./markdown-enhanced-chrome-path.png)
