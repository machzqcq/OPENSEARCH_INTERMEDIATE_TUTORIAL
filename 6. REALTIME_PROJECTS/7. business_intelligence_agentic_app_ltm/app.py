...
def initialize_opensearch():
    ...
    try:
        ...
        # Configure cluster settings (including memory feature for conversational AI)
        cluster_settings = {
            "persistent": {
        ...
        print("✅ OpenSearch cluster settings configured with memory feature enabled")
        
        return True, "✅ OpenSearch client initialized successfully (with conversation memory support)", os_client, ml_client
    except Exception as e:
...
def format_metadata_for_llm(metadata_list):
    """Format metadata into readable context for LLM with explicit column name casing"""
    ...
    for item in metadata_list:
        ...
        col_type = item.get('data_type', '')
        col_desc = item.get('inferred_column_description', '')
    ...
    # Format as text with explicit SQL usage instructions
    context = ""
    context += "="*70 + "\n"
    context += "AVAILABLE TABLES AND COLUMNS (ONLY THESE EXIST IN DATABASE)\n"
    context += "="*70 + "\n"
    
    for table_name, table_info in tables.items():
        context += f"\n📋 Table: {table_name}\n"
        context += f"Description: {table_info['description']}\n"
        context += f"\n✓ AVAILABLE COLUMNS (Total: {len(table_info['columns'])}):\n"
        context += "-"*70 + "\n"
        
        for idx, col in enumerate(table_info['columns'], 1):
            # Show both the actual name and how to use it in SQL
            context += f"{idx}. column_name=\"{col['name']}\" → USE IN SQL AS: {col['sql_usage']} ({col['type']})\n"
            if col['description']:
                context += f"   💡 {col['description']}\n"
        
        context += "-"*70 + "\n"
        context += f"⚠️  ONLY these {len(table_info['columns'])} columns exist in {table_name}\n"
        context += f"❌ DO NOT use any column names not listed above for {table_name}\n"
        context += "\n"
    
    context += "="*70 + "\n"
    context += "⚠️  IMPORTANT: This is the COMPLETE list of available columns\n"
    context += "❌ NO OTHER COLUMNS exist beyond what is shown above\n"
    context += "✓  ONLY use columns from this list in your SQL query\n"
    context += "="*70 + "\n"
...
def generate_sql_with_deepseek(query_text, metadata_context, conversation_history=None):
    ...
    try:
        ...
        if conversation_history:
            conversation_context = format_conversation_history_for_llm(conversation_history)
...
3. The schema above shows "SQL_USAGE" which tells you EXACTLY how to reference each column
   ...
   - If SQL_USAGE shows: "PRODUCTID" → use: "PRODUCTID" (with quotes, uppercase)

4. NEVER guess or change column casing - copy EXACTLY from SQL_USAGE in schema

**Example 1 - Correct Column Casing**:
If schema shows:
...
If schema shows for sales.salesorderdetail:
  - column_name="salesorderid" → USE IN SQL AS: salesorderid
  - column_name="orderqty" → USE IN SQL AS: orderqty
  - column_name="unitprice" → USE IN SQL AS: unitprice
  
✓ CORRECT (calculate from existing columns):
  SELECT salesorderid, orderqty * unitprice AS line_total
  FROM sales.salesorderdetail
  
❌ WRONG (column doesn't exist):
  SELECT salesorderid, linetotal FROM sales.salesorderdetail
  -- ERROR: column "linetotal" does not exist!
  
**Key Point**: If a column you need doesn't exist, you must:
1. Calculate it from available columns (e.g., qty * price)
2. Or find a similar column in the schema
3. NEVER assume a column exists if not in schema
...
- Use schema.table notation (e.g., sales.customer, production.product)
- Include appropriate JOINs if needed

**CRITICAL COLUMN VALIDATION**:
- ONLY use columns that are EXPLICITLY listed in the "Available Database Schema" above
- DO NOT guess, infer, or assume column names that are not shown in the schema
- If a column is not listed in the schema, it does NOT exist in the database
- Before using ANY column, verify it appears in the schema above with exact name
- If you need a column that doesn't exist (like "linetotal", "total", "amount"), you MUST:
  * Check the schema for similar columns (e.g., "LineTotal", "TotalDue", "OrderQty")
  * Use ONLY the exact column names shown in SQL_USAGE
  * DO NOT create or assume column names

**COLUMN NAME MATCHING**:
- Check the EXACT column_name in the schema above and match the casing precisely
- If the column is all lowercase in schema (e.g., column_name="productid"), use it WITHOUT quotes: productid
- If the column has ANY uppercase or mixed case (e.g., column_name="ProductID"), use it WITH quotes: "ProductID"
- Copy the SQL_USAGE format exactly as shown in the schema

**VALIDATION CHECKLIST** - Before finalizing SQL:
1. ✓ Every column in SELECT clause exists in schema
2. ✓ Every column in WHERE clause exists in schema
3. ✓ Every column in JOIN condition exists in schema
4. ✓ Every column in GROUP BY exists in schema
5. ✓ Every column in ORDER BY exists in schema
6. ✓ Column names match SQL_USAGE exactly (including quotes)

- For follow-up questions, build upon or modify the previous SQL query as appropriate
...
def text_to_sql_pipeline(os_client, query_text, index_name, model_id, top_k=10):
    """Complete text-to-SQL pipeline with RAG and conversation memory"""
    try:
        # Step 1: Get or create conversation memory
        memory_id, mem_error = get_or_create_memory(os_client)
        conversation_history = []
        
        if memory_id and not mem_error:
            # Step 2: Retrieve conversation history
            conversation_history, hist_error = retrieve_conversation_history(os_client, memory_id, max_messages=5)
            if hist_error:
                print(f"DEBUG: Could not retrieve history: {hist_error}")
                conversation_history = []
        
        # Step 3: Retrieve relevant metadata
        ...
        # Step 5: Generate SQL with conversation context
        sql_query, error = generate_sql_with_deepseek(query_text, metadata_context, conversation_history)
        
        if error:
            return None, error, None, conversation_history
        
        # Step 6: Store this interaction in memory
        if memory_id:
            store_conversation_message(
                os_client, 
                memory_id, 
                query_text, 
                f"Generated SQL query: {sql_query[:100]}...",
                metadata_context,
                sql_query
            )
        
        return sql_query, None, metadata_context, conversation_history
...
def create_conversation_memory(os_client):
    """Create a new conversation memory for the session"""
    try:
        response = os_client.transport.perform_request(
            'POST',
            '/_plugins/_ml/memory',
            body={
                "name": f"bi_conversation_{int(time.time())}",
                "type": "conversation"
            }
        )
        memory_id = response.get('memory_id')
        print(f"DEBUG: Created conversation memory with ID: {memory_id}")
        return memory_id, None
    except Exception as e:
        print(f"DEBUG: Error creating conversation memory: {str(e)}")
        return None, f"Error creating memory: {str(e)}"

def get_or_create_memory(os_client):
    """Get existing memory or create new one"""
    memory_id = global_state.get('conversation_memory_id')
    ...
    if memory_id:
        ...
        try:
            ...
            if response:
                print(f"DEBUG: Using existing memory: {memory_id}")
                return memory_id, None
        except Exception as e:
            print(f"DEBUG: Existing memory not found, creating new one: {str(e)}")
    
    # Create new memory
    memory_id, error = create_conversation_memory(os_client)
    if error:
        return None, error
    
    global_state['conversation_memory_id'] = memory_id
    return memory_id, None

def store_conversation_message(os_client, memory_id, user_query, assistant_response, metadata_context, sql_query):
    """Store a conversation message pair in memory"""
    try:
        ...
        interaction = {
            ...
            "origin": "bi_rag_app",
            "additional_info": {
                "metadata_context": metadata_context[:500] if metadata_context else "",  # Truncate if too long
                "sql_query": sql_query
            }
        }
        ...
        print(f"DEBUG: Stored message with ID: {message_id}")
        ...
        return None, f"Error storing message: {str(e)}"

def retrieve_conversation_history(os_client, memory_id, max_messages=5):
    """Retrieve recent conversation history from memory"""
    try:
        response = os_client.transport.perform_request(
            'GET',
            f'/_plugins/_ml/memory/{memory_id}/messages',
            params={'max_results': max_messages}
        )
        
        messages = response.get('messages', [])
        print(f"DEBUG: Retrieved {len(messages)} messages from memory")
        
        # Format conversation history
        conversation_context = []
        for msg in messages:
            interaction = {
                'question': msg.get('input', ''),
                'response': msg.get('response', ''),
                'sql_query': msg.get('additional_info', {}).get('sql_query', ''),
                'timestamp': msg.get('create_time', '')
            }
            conversation_context.append(interaction)
...
def format_conversation_history_for_llm(conversation_history):
    ...
    for i, interaction in enumerate(conversation_history[-3:], 1):  # Last 3 interactions
        formatted += f"\n{i}. User asked: \"{interaction['question']}\"\n"
        if interaction.get('sql_query'):
            formatted += f"   Generated SQL: {interaction['sql_query'][:200]}...\n"
        if interaction.get('response'):
            formatted += f"   Response: {interaction['response'][:100]}...\n"
    
    return formatted

def format_conversation_history_for_display(conversation_history):
    """Format conversation history for UI display"""
    if not conversation_history:
        return "📝 No previous conversation in this session.\n\nThis is the first query in the conversation."
    
    formatted = f"💬 **Conversation History** ({len(conversation_history)} previous interactions)\n"
    formatted += "=" * 70 + "\n\n"
    
    for i, interaction in enumerate(conversation_history, 1):
        formatted += f"**Turn {i}:**\n"
        formatted += f"👤 **User**: {interaction['question']}\n"
        if interaction.get('sql_query'):
            # Show truncated SQL
            sql_preview = interaction['sql_query'][:150]
            if len(interaction['sql_query']) > 150:
                sql_preview += "..."
            formatted += f"💻 **SQL**: `{sql_preview}`\n"
        formatted += f"🕒 **Time**: {interaction.get('timestamp', 'N/A')}\n"
        formatted += "-" * 70 + "\n\n"
...
def clear_conversation_memory():
    ...
    global_state['conversation_memory_id'] = None
    global_state['conversation_history'] = []
    return "✅ Conversation memory cleared. Next query will start a new conversation."

# ============================================================================
# LONG-TERM MEMORY (LTM) MANAGEMENT
# ============================================================================

def create_ltm_index(os_client, model_id):
    """Create long-term memory index with vector embeddings for query similarity search"""
    try:
        ltm_index = global_state['ltm_index_name']
        ltm_pipeline = global_state['ltm_pipeline_name']
        
        # Delete existing index if it exists
        if os_client.indices.exists(index=ltm_index):
            print(f"DEBUG: Deleting existing LTM index: {ltm_index}")
            os_client.indices.delete(index=ltm_index)
        
        # Delete existing pipeline if it exists
        try:
            os_client.ingest.delete_pipeline(id=ltm_pipeline)
        except:
            pass
        
        # Create embedding pipeline for user queries
        pipeline_body = {
            "description": "Embedding pipeline for long-term memory queries",
            "processors": [
                {
                    "text_embedding": {
                        "model_id": model_id,
        ...
        os_client.ingest.put_pipeline(id=ltm_pipeline, body=pipeline_body)
        print(f"DEBUG: Created LTM pipeline: {ltm_pipeline}")
        ...
        os_client.indices.create(index=ltm_index, body=index_body)
        print(f"DEBUG: Created LTM index: {ltm_index}")
        
        return True, f"✅ Long-term memory index created: {ltm_index}"
        
    except Exception as e:
        print(f"DEBUG: Error creating LTM index: {str(e)}")
        import traceback
        traceback.print_exc()
        return False, f"❌ Failed to create LTM index: {str(e)}"

def search_similar_queries_in_ltm(os_client, user_query, model_id, top_k=5):
    """Search for similar queries in long-term memory using semantic search"""
    try:
        ltm_index = global_state['ltm_index_name']
        
        # Check if index exists
        if not os_client.indices.exists(index=ltm_index):
            print("DEBUG: LTM index doesn't exist yet")
            return [], None
        
        # Check document count
        count_response = os_client.count(index=ltm_index)
        doc_count = count_response['count']
        print(f"DEBUG: LTM index has {doc_count} documents")
        
        if doc_count == 0:
            return [], None
        
        # Perform semantic search for similar queries
        search_body = {
            "size": top_k,
            "query": {
                "neural": {
            ...
            "_source": ["user_query", "generated_sql", "timestamp", "metadata_context"]
        }
        
        print(f"DEBUG: Searching LTM for similar queries to: '{user_query}'")
        results = os_client.search(index=ltm_index, body=search_body)
        ...
        for hit in results['hits']['hits']:
            ...
            similar_queries.append({
                'query': source.get('user_query', ''),
                'sql': source.get('generated_sql', ''),
                'timestamp': source.get('timestamp', ''),
                'score': similarity_score
            })
        
        print(f"DEBUG: Found {len(similar_queries)} similar queries in LTM")
        return similar_queries, None
        
    except Exception as e:
        print(f"DEBUG: Error searching LTM: {str(e)}")
        import traceback
        traceback.print_exc()
        return [], f"Error searching LTM: {str(e)}"

def store_query_in_ltm(os_client, user_query, generated_sql, metadata_context, execution_success=True):
    """Store user query and generated SQL in long-term memory"""
    try:
        ltm_index = global_state['ltm_index_name']
        
        # Check if index exists
        if not os_client.indices.exists(index=ltm_index):
            print("DEBUG: LTM index doesn't exist, skipping storage")
            return False, "LTM index not initialized"
        ...
        doc = {
            ...
            "metadata_context": metadata_context[:500] if metadata_context else "",  # Truncate
            "execution_success": execution_success
        }
        
        # Index document (embedding will be auto-generated by pipeline)
        response = os_client.index(
            index=ltm_index,
            body=doc,
            refresh='wait_for'  # Wait for doc to be searchable
        )
        
        doc_id = response['_id']
        print(f"DEBUG: Stored query in LTM with doc ID: {doc_id}")
        
        return True, None
        
    except Exception as e:
        print(f"DEBUG: Error storing in LTM: {str(e)}")
        return False, f"Error storing in LTM: {str(e)}"

def format_similar_queries_for_display(similar_queries):
    """Format similar queries for UI display"""
    if not similar_queries:
        return "No similar queries found in history."
    
    formatted = f"🔍 **Found {len(similar_queries)} Similar Queries in History**\n"
    formatted += "="*70 + "\n\n"
    
    for i, query_data in enumerate(similar_queries, 1):
        formatted += f"**Option {i}** (Similarity: {query_data['score']:.2f})\n"
        formatted += f"📝 **Query**: {query_data['query']}\n"
        formatted += f"💻 **SQL**: {query_data['sql'][:150]}{'...' if len(query_data['sql']) > 150 else ''}\n"
        formatted += f"🕒 **Date**: {query_data['timestamp']}\n"
        formatted += "-"*70 + "\n\n"
    
    formatted += f"**Option {len(similar_queries) + 1}**: 🆕 Generate new SQL (something else)\n"
    formatted += "="*70 + "\n"
...
def setup_opensearch(progress=gr.Progress()):
    """Initialize OpenSearch and register model with LTM support"""
    try:
        progress(0, desc="🔄 Starting OpenSearch setup...")
        status_updates = []
        
        # Step 1: Initialize OpenSearch
        progress(0.25, desc="🔌 Connecting to OpenSearch cluster...")
        ...
        status_updates.append(f"✅ Step 1/3: {message}")
        
        # Step 2: Register embedding model
        progress(0.5, desc="📦 Checking/Registering embedding model...")
        success, msg, model_id = register_embedding_model(ml_client)
        if not success:
            return f"{status_updates[0]}\n❌ Step 2/3: {msg}"
        
        global_state['model_id'] = model_id
        status_updates.append(f"\n📋 Step 2/3: Embedding Model Status")
        status_updates.append(f"   {msg}")
        
        # Step 3: Create Long-Term Memory index
        progress(0.75, desc="🧠 Setting up Long-Term Memory index...")
        success, ltm_msg = create_ltm_index(os_client, model_id)
        if not success:
            status_updates.append(f"\n⚠️  Step 3/3: {ltm_msg}")
            status_updates.append(f"   (LTM optional - continuing without it)")
        else:
            status_updates.append(f"\n🧠 Step 3/3: Long-Term Memory")
            status_updates.append(f"   {ltm_msg}")
        
        progress(1.0, desc="✅ OpenSearch setup complete!")
        status_updates.append(f"\n💡 Embedding pipeline will be created during ingestion (Tab 5)")
        status_updates.append(f"\n💾 Long-term memory is ready to cache your queries!")
...
def check_similar_queries_ui(query_text):
    """Check for similar queries in LTM and present options as checkboxes"""
    os_client = global_state.get('os_client')
    model_id = global_state.get('model_id')
    
    if not os_client or not model_id:
        return "❌ OpenSearch not initialized", "", gr.update(visible=False), gr.update(visible=False)
    
    if not query_text or query_text.strip() == "":
        return "❌ Please enter a question", "", gr.update(visible=False), gr.update(visible=False)
    
    try:
        # Search for similar queries in LTM
        similar_queries, error = search_similar_queries_in_ltm(os_client, query_text, model_id, top_k=5)
        
        if error:
            return f"⚠️ {error}", "", gr.update(visible=False), gr.update(visible=False)
        
        if not similar_queries or len(similar_queries) == 0:
            # No similar queries found, proceed with normal flow
            return "✅ No similar queries found. Generating new SQL...", "", gr.update(visible=False), gr.update(visible=True)
        
        # Store similar queries in global state for later use
        global_state['similar_queries_cache'] = similar_queries
        
        # Format for display
        display_text = format_similar_queries_for_display(similar_queries)
        
        # Create checkbox options with detailed info
        options = []
        for i, q in enumerate(similar_queries):
            query_preview = q['query'][:70] + "..." if len(q['query']) > 70 else q['query']
            options.append(f"Query {i+1} (Score: {q['score']:.2f}): {query_preview}")
        
        # Always add "something else" option
        options.append("🆕 Something else - Generate new SQL")
        
        status = f"🔍 Found {len(similar_queries)} similar queries in history!\n\n"
        status += "👇 Check one or more queries below to view their SQL, or select 'Something else' to generate new SQL:"
        
        return status, display_text, gr.update(choices=options, value=[], visible=True), gr.update(visible=False)
        
    except Exception as e:
        print(f"DEBUG: Error in check_similar_queries_ui: {str(e)}")
        import traceback
        traceback.print_exc()
        return f"❌ Error: {str(e)}", "", gr.update(visible=False), gr.update(visible=False)

def check_similar_queries_and_generate_ui(query_text):
    """Check for similar queries in LTM and auto-generate SQL if none found"""
    os_client = global_state.get('os_client')
    model_id = global_state.get('model_id')
    ...
    try:
        ...
        if not similar_queries or len(similar_queries) == 0:
            # No similar queries found - automatically generate new SQL
            status_msg = "✅ No similar queries found in LTM. Generating new SQL..."
            sql_status, sql_output, conv_history = generate_new_sql_ui(query_text)
            return status_msg, "", gr.update(visible=False), gr.update(visible=False), sql_status, sql_output, conv_history
        ...
        # Return with visible checkboxes and button, empty SQL outputs
        return status, display_text, gr.update(choices=options, value=[], visible=True), gr.update(visible=True), "", "", ""
        
    except Exception as e:
        print(f"DEBUG: Error in check_similar_queries_and_generate_ui: {str(e)}")
        import traceback
        traceback.print_exc()
        return f"❌ Error: {str(e)}", "", gr.update(visible=False), gr.update(visible=False), "", "", ""

def handle_query_selection_ui(query_text, selected_options):
    """Handle user selection of similar queries (checkbox list) or generate new SQL"""
    os_client = global_state.get('os_client')
    model_id = global_state.get('model_id')
    ...
    similar_queries = global_state.get('similar_queries_cache', [])
    
    # Check if user selected "something else" option
    has_something_else = any("Something else" in opt for opt in selected_options)
    
    if has_something_else:
        # User wants to generate new SQL
        return generate_new_sql_ui(query_text)
    
    # User selected one or more similar queries - show their SQL
    try:
        status_msg = "✅ Displaying SQL from selected cached queries:\n\n"
        combined_sql = ""
        history_display = "💾 Using cached SQL from long-term memory\n\n"
        
        for selected_option in selected_options:
            # Extract query number from checkbox label (e.g., "Query 1 (Score: 0.95): ...")
            query_num = int(selected_option.split("(")[0].replace("Query", "").strip()) - 1
            
            if query_num < 0 or query_num >= len(similar_queries):
                continue
            
            # Get the selected query data
            selected_query_data = similar_queries[query_num]
            
            # Append to combined output
            status_msg += f"📊 Query {query_num + 1} (Score: {selected_query_data['score']:.2f})\n"
            status_msg += f"   📅 Date: {selected_query_data['timestamp']}\n"
            
            combined_sql += f"-- ========================================\n"
            combined_sql += f"-- Query {query_num + 1}: {selected_query_data['query']}\n"
            combined_sql += f"-- Similarity Score: {selected_query_data['score']:.2f}\n"
            combined_sql += f"-- Date: {selected_query_data['timestamp']}\n"
            combined_sql += f"-- ========================================\n\n"
            combined_sql += selected_query_data['sql'] + "\n\n\n"
            
            history_display += f"Query {query_num + 1}: {selected_query_data['query']}\n"
            history_display += f"  Score: {selected_query_data['score']:.2f} | Date: {selected_query_data['timestamp']}\n\n"
        
        status_msg += f"\n⚡ Skipped LLM call (saved time and cost!)\n"
        status_msg += f"📝 Showing {len(selected_options)} cached SQL {'query' if len(selected_options) == 1 else 'queries'}"
        
        # Store the first selected SQL in global state for execution
        ...
        history_display += f"\n💡 Tip: The first selected query's SQL is ready for execution in Tab 7"
        
        return status_msg, combined_sql, history_display
        
    except Exception as e:
        print(f"DEBUG: Error in handle_query_selection_ui: {str(e)}")
        return f"❌ Error: {str(e)}", "", ""

def generate_new_sql_ui(query_text):
    """Generate new SQL using LLM and conversation memory"""
    os_client = global_state.get('os_client')
    model_id = global_state.get('model_id')
    index_name = global_state.get('index_name')
    ...
    try:
        sql_query, error, metadata_context, conversation_history = text_to_sql_pipeline(
        ...
        store_success, store_error = store_query_in_ltm(
            os_client, query_text, sql_query, metadata_context, execution_success=True
        )
        
        # Format conversation history for display
        history_display = format_conversation_history_for_display(conversation_history)
        
        # Build status message
        status_msg = "✅ SQL generated successfully"
        if store_success:
            status_msg += "\n💾 Saved to long-term memory for future use"
        if conversation_history:
            status_msg += f"\n💬 Using context from {len(conversation_history)} previous interactions"
        else:
            status_msg += "\n📝 Starting new conversation"
        
        return status_msg, sql_query, history_display
        
    except Exception as e:
        print(f"DEBUG: Error in generate_new_sql_ui: {str(e)}")
        import traceback
        traceback.print_exc()
        return f"❌ Error: {str(e)}", "", ""
        ...
        **Complete workflow for Text-to-SQL-to-Visualization-to-Insights using RAG**
        ...
        with gr.Tabs() as tabs:
            ...
            with gr.Tab("6️⃣ Ask Questions"):
                gr.Markdown("## Natural Language to SQL with Long-Term Memory")
                gr.Markdown("""
                **Step 6**: Ask business questions with smart query caching!
                
                **🧠 How Long-Term Memory Works**:
                1. Enter your question in plain English
                2. System searches for **similar past queries** using vector embeddings
                3. **Select one or more cached queries** to view their SQL (instant retrieval!)
                4. Or check **"Something else"** to generate new SQL with LLM
                5. **Cached queries skip LLM call** - instant SQL retrieval!
                6. New queries generate SQL using conversation + metadata (RAG)
                7. All new queries are **automatically cached** for future use
                
                **💡 Features:**
                - **Review multiple queries**: Select several cached queries to compare their SQL
                - **Save time & cost**: Reuse SQL from similar past queries
                - **Consistency**: Same question = same SQL every time
                - **Conversational memory**: Follow-up questions maintain context
                - **Smart matching**: Semantic search finds similar queries even with different wording
                
                **Example**: 
                - First time: "Show top 10 customers by revenue" → Generates SQL → Stores in cache
                - Next time: "Top 10 customers sorted by revenue" → Finds cached SQL → Select checkbox → Instant result!
                - Compare queries: Select multiple checkboxes to see SQL for all selected queries side-by-side
                """)
                ...
                with gr.Row():
                    check_similar_btn = gr.Button("🔍 Check Similar Queries", variant="primary")
                    clear_memory_btn = gr.Button("🗑️ Clear Conversation", variant="secondary")
                
                similar_queries_status = gr.Textbox(label="Status", lines=2)
                similar_queries_display = gr.Textbox(label="💾 Similar Queries Found", lines=8, visible=False)
                ...
                with gr.Row():
                    use_selected_btn = gr.Button("✅ View Selected SQL / Generate New", variant="primary", visible=False)
                
                sql_status = gr.Textbox(label="SQL Generation Status", lines=3)
                ...
                clear_memory_status = gr.Textbox(label="Clear Memory Status", visible=False)
                
                # Step 1: Check for similar queries (and auto-generate if none found)
                check_similar_btn.click(
                    check_similar_queries_and_generate_ui,
                    inputs=query_input,
                    outputs=[similar_queries_status, similar_queries_display, query_selection, use_selected_btn, sql_status, sql_output, conversation_history_display]
                )
                
                # Step 2: Handle user selection (cached or new)
                use_selected_btn.click(
                    handle_query_selection_ui,
                    inputs=[query_input, query_selection],
                    outputs=[sql_status, sql_output, conversation_history_display]
                )
            ...
            with gr.Tab("7️⃣ Execute Query"):
                ...
                **Step 7**: Execute the generated SQL query on the database.
                ...
                - See the conversation history that influenced this query
                ...
                with gr.Row():
                    ...
                    with gr.Column():
                        exec_conversation_display = gr.Textbox(
                            label="💬 Conversation Context", 
                            lines=10,
                            placeholder="Conversation context will appear here..."
                        )
                ...
                # Also sync conversation history
                conversation_history_display.change(
                    lambda x: x, 
                    inputs=conversation_history_display, 
                    outputs=exec_conversation_display
                )
